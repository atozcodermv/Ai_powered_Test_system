...
class UserServiceImplTest {
    ...
    @Test
    void getUsersByRoleAndTeacherAndStatus() {
        // Given
        String role = "STUDENT";
        User teacher = new User();
        String status = "ACTIVE";
        List<User> expectedUsers = new ArrayList<>();
        when(userRepository.findByRoleAndTeacherAndStatus(role, teacher, status)).thenReturn(expectedUsers);

        // When
        List<User> result = userService.getUsersByRoleAndTeacherAndStatus(role, teacher, status);

        // Then
        assertEquals(expectedUsers, result);
        verify(userRepository, times(1)).findByRoleAndTeacherAndStatus(role, teacher, status);
    }
}
