...
public class StudentAnswerResource {
	...
	public ResponseEntity<QuestionsResponseDto> addStudentAnswers(StudentAnswerRequest request) {
		...
		if (request.isAutoSubmit() && request.getViolationReason() != null && !request.getViolationReason().isEmpty()) {
		    ...
		    cheatingLog.setTimestamp(answerSubmittedTime);
		    ...
		    if (teacher != null && teacher.getEmailId() != null) {
		        try {
		            this.emailService.sendCheatingNotificationEmail(
		                ...
		                DateTimeUtils.getProperDateTimeFormatFromEpochTime(answerSubmittedTime)
		            );
		        } catch (Exception e) {
		...
		if (request.isAutoSubmit() && request.getViolationReason() != null && !request.getViolationReason().isEmpty()) {
		    response.setResponseMessage("Exam Auto-Submitted Details Saved due to: " + request.getViolationReason());
		} else {
	...
	private String populateStudentExamResultAndMailResultToStudent(List<Question> questions, Exam exam, User student,
		...
		examResult.setEvaluationStatus(EvaluationStatus.EVALUATED.value());
		...
		try {
			this.emailService.sendExamResultEmail(student.getEmailId(), student.getFirstName() + " " + student.getLastName(), addedResult, universityName);
			LOG.info("Exam Result Mail Sent to " + student.getEmailId());
		} catch (Exception e) {
			LOG.error("Failed to send Exam Result Mail to " + student.getEmailId(), e);
		}
	...
	public String makeEmailBody(ExamResult examResult, String resultMessage) {
        ...
        // but now the system uses Centralized EmailService.sendExamResultEmail
