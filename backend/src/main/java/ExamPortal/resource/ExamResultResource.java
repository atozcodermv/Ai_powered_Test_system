...
public class ExamResultResource {
	...
	public ResponseEntity<ExamResultResponse> fetchStudentExamResult(int studentId) {
		...
		for (ExamResult result : results) {
			...
			if (Constants.ExamType.DESCRIPTIVE.value().equalsIgnoreCase(result.getExam().getExamType())) {
				populateDescriptiveAnswers(result, student);
			} else if (!result.getExam().getQuestions().isEmpty()) {
	...
	public ResponseEntity<ExamResultResponse> fetchStudentExamResultGradeWise(int gradeId) {
		...
		for (ExamResult result : results) {
			...
			if (Constants.ExamType.DESCRIPTIVE.value().equalsIgnoreCase(result.getExam().getExamType())) {
				populateDescriptiveAnswers(result, result.getStudent());
			} else if (!result.getExam().getQuestions().isEmpty()) {
...
<<<<<<< HEAD
	private void populateDescriptiveAnswers(ExamResult result, User student) {
=======
	public ResponseEntity<java.util.List<Object[]>> fetchExamPerformanceByStudentId(int studentId) {
		LOG.info("Request received for fetching exam performance for student: " + studentId);
		return new ResponseEntity<>(this.examResultService.getExamPerformanceByStudentId(studentId), HttpStatus.OK);
	}

	private void populateDescriptiveResult(ExamResult result) {
>>>>>>> 8c562aa (added the admin dashbord)
		List<DescriptiveQuestion> questions = this.descriptiveQuestionService.getQuestionsByExam(result.getExam());
		List<DescriptiveAnswer> answers = this.descriptiveAnswerService
				.getAnswersByExamAndStudent(result.getExam().getId(), student.getId());
		...
		result.getExam().setDescriptiveQuestions(questions);
	}
